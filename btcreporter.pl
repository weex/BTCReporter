...
if( $help ) { print "BTCReporter v0.7. (c) 2012 David Sterry\n\tUsage:
...
if ( !$nomtgox and -e 'history_BTC.csv' and -e 'history_USD.csv' ) {
  ...
  while (my $row = $csv->getline ($fh)) {
    ...
    # BTC sold
      ...
      my $delta_fiat = $quantity * $price;
      $total{'USD'} += $delta_fiat;
      $sells{'USD'} += $delta_fiat;
      $total{'BTC'} -= $quantity;
      $total_by_pair{'USD'} -= $quantity;
      $sells_by_pair{'USD'} -= $quantity;
...
if ( !$noth and -e 'TradeHill-TradeHistory.csv' ) {
  ...
  while (my $row = $csv->getline ($fh)) {
	...
	$total{'BTC'} -= $quantity;
	$total{$fiat_currency} += $delta_fiat;
	$sells{$fiat_currency} += $delta_fiat;
	$total_fees{$fiat_currency} += $fee{$fiat_currency};
        $total_by_pair{$fiat_currency} -= $quantity;
        $sells_by_pair{$fiat_currency} -= $quantity;
...
my @summary;
foreach( reverse keys %total ) {
  $summary[$index]->[0] = "$_";
  $summary[$index]->[1] = $total{$_};

  if( $_ ne 'BTC' ) { 
    $summary[$index]->[2] = $total_by_pair{$_};
    $summary[$index]->[3] = abs($total{$_}/$total_by_pair{$_});
    $summary[$index]->[4] = abs($buys{$_});
    $summary[$index]->[5] = $buys_by_pair{$_};
    $summary[$index]->[6] = abs($sells{$_});
    $summary[$index]->[7] = abs($sells_by_pair{$_});
  } else { 
    $summary[$index]->[2] = 'n/a';
    $summary[$index]->[3] = 'n/a';
    $summary[$index]->[4] = 'n/a';
  ...
  $summary[$index]->[8] = $total_fees{$_};

  if( $_ ne 'BTC' ) { 
    $summary[$index]->[9] = $total_fees_in_fiat{$_};
  } else {
    $summary[$index]->[9] = 'n/a';
  }
  $index++;
...
@headers = ('','Net change','Net BTC','Avg price','Used to purchase','BTC purchased','Gained from sale','BTC sold','Fees','BTC fees converted');
...
if($summary) {
  print "Generated by BTCReporter\n";
  print sprintf( "%5s %14.14s %12.12s %14.12s %17.17s %14.14s %17.17s %14.14s %14.14s %18.18s\n",@headers); 
